...
export default function ArtistPendingRequests() {
  ...
  for (let i = 0; i < divCount; i++) {
    ...
    divElements.push(
      <div key={i} className="requestContainer">
        <img
          src={`${BASE_URL}/postData/uploads/image/${eventList[i]["organizerImage"]}`}
          className="profile"
        ></img>
        <div className="eventDetails">
          ...
          <p class="eventType">
            <img src={eventtype} className="EventIconPendingRequest1"></img>
            {eventList[i]["eventName"]}
          </p>
          <p class="eventDate">
            <FontAwesomeIcon
              icon={faCalendarDays}
              id="CalenderIconPendingRequest"
            />
            {eventList[i]["date"]}
          </p>
          <p class="venue">
